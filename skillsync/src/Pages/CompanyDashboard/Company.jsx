...
export const Company = () => {
  ...
  return (
    <div className="Content">
      ...
      <div style={{ border: '2px solid #ccc', borderRadius: '50%', width: '150px', height: '150px', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#f0f0f0' }}>
        ...
        <div className="profileUpdate" style={{ flexDirection: "row"}}>
      ...
      <Modal open={formOpen} onClose={handleFormClose}>
        <Box sx={{ ...modalStyle, width: 400 }}>
          <form className="company-info" onSubmit={handleSubmit}>
            ...
            {["companyName", "address", "telephone", "email", "establishedYear"].map((field) => (
              <div className="form-group" key={field}>
                <label htmlFor={field}>{field.replace(/([A-Z])/g, ' $1')}</label>
                <input
                  type={field === "establishedYear" ? "number" : field === "email" ? "email" : "text"}
                  id={field}
                  name={field}
                  value={profile[field]}
